...
function sha256Hex(input: string): string {
  ...
  const paddingLength = (message.length % 64 < 56 ? 56 : 120) - (message.length % 64)
  ...
  for (let chunkStart = 0; chunkStart < padded.length; chunkStart += 64) {
    for (let wordIndex = 0; wordIndex < 16; wordIndex += 1) {
      words[wordIndex] = view.getUint32(chunkStart + wordIndex * 4, false)
    }
    ...
    for (let round = 0; round < 64; round += 1) {
      ...
      const temp1 = (h + bigSigma1 + choice + sha256RoundConstants[round]! + words[round]!) >>> 0
